...
fn reconstruct_failure() -> io::Result<AxialFailure> {
    ...
    let (first_output_domain, first_output) = boundary(Sign::Output, DIRECTION, &first);
    let (second_input_domain, second_input) = boundary(Sign::Input, DIRECTION, &second);
    ...
    let (_, actual_input) = boundary(Sign::Input, DIRECTION, &pasted.tip);
    let (_, first_input) = boundary(Sign::Input, DIRECTION, &first);
    let expected_input = Embedding::compose(&first_input, &pasted.inl);
    let (_, actual_output) = boundary(Sign::Output, DIRECTION, &pasted.tip);
    let (_, second_output) = boundary(Sign::Output, DIRECTION, &second);
...
fn assert_transverse_equations(
    ...
    for sign in [Sign::Input, Sign::Output] {
        let (_, actual) = boundary(sign, 1, &pasted.tip);
        let (_, first_boundary) = boundary(sign, 1, first);
        let (_, second_boundary) = boundary(sign, 1, second);
