...
use ofposets::{
    DirectionImage, Embedding, FramedPoset, Sign, SignedPermutation, normalize,
    random_framed_poset, transform,
};
...
fn main() -> io::Result<()> {
    ...
    let run_result = thread::scope(|scope| {
        for worker in 0..worker_count {
            ...
            scope.spawn(move || {
                if let Err(error) = sample_worker(
                    worker,
                    seed,
                    &symmetries,
                    &state,
...
fn sample_worker(
    ...
    while !stop.load(Ordering::Acquire) {
        ...
        if increment_cached_orbit(state, &normal)? {
            continue;
        }

        let analysis = analyze_orbit(&normal, symmetries)?;
        register_or_increment_orbit(state, analysis)?;
    }
